---
slug: /data-structures/disjoint-set-union
discussionId: /data-structures/disjoint-set-union
...
This application is quite important, because nearly the same problem appears in [Kruskal's algorithm for finding a minimum spanning tree](./graph/mst-kruskal).
Using DSU we can [improve](./graph/mst-kruskal-with-dsu) the $O(m \log n + n^2)$ complexity to $O(m \log n)$.
...
The algorithm for finding the LCA is discussed in the article [Lowest Common Ancestor - Tarjan's off-line algorithm](./graph/lca-tarjan).
This algorithm compares favorable with other algorithms for finding the LCA due to its simplicity (especially compared to an optimal algorithm like the one from [Farach-Colton and Bender](./graph/lca-farachcoltonbender)).
